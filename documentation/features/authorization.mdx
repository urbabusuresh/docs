...
Basic Authentication is one of the simplest methods to secure your API requests in wso2apisuite. It works by requiring a username and password to be sent along with each request. To get started, head to the **Authorization** tab and select `Basic Auth` as your authentication type. You'll then input your verified **username** and **password**. When you send the request, wso2apisuite will automatically encode your credentials in the format: 
...
Bearer Tokens are used to authenticate requests using an access token, which can be a simple string or a JWT (JSON Web Token). To set it up, choose `Bearer` from the **Authorization** tab and enter your **access token**. wso2apisuite will include the token in the request header as:

```
Bearer <your-access-token>
```

<Info>Be aware that if your token expires, you'll need to refresh it manually and update it in wso2apisuite. For better security, consider storing your token as an environment variable so that you can easily reference it in your requests.</Info>

## API Key
API Keys are unique identifiers that help authenticate your requests to an API. In wso2apisuite, you can set up `API Key` authentication by selecting it from the **Authorization** tab. Here, you'll need to fill in two fields: the **Key**, which is the name the API expects (like `api_key` or `Authorization`), and the **Value**, which is your actual API Key. You can also choose whether to pass this API Key as a **Header** or as a **Query Parameter**.
...
<Warning> OAuth 2.0 workflows are not fully supported in the wso2apisuite Desktop application and are currently a work in progress. We're actively working on it, and it will function as intended soon!</Warning>
...
When using OAuth 2.0 authorization with wso2apisuite, you can utilize the following grant types:
...
To use the `Authorization Code` grant type, ensure the [Callback URL](#oauth-callback-url-configuration) is correctly configured (either `http://172.16.111.83:3333/oauth` for wso2apisuite Cloud or `<your-domain>/oauth` for self-hosted editions). Next, provide the **Authorization Endpoint** and **Access Token Endpoint** from the API provider, along with the **Client ID** (and the Client Secret if you choose to use it). You can also specify the desired permissions in the **Scope** field and pass the token as a **Header** or as a **Query Parameter**. Once you've filled in these fields, click Generate Token to get your access token.

<Tip> If your API provider supports token refreshing, you can use the **"Refresh Token"** button in wso2apisuite to obtain a new access token without requiring the user to log in again.</Tip>
...
<Note> 
You can now choose how the **client credentials** (Client ID and Client Secret) are sent to the server. wso2apisuite supports both:
...
To implement the `Password` credentials grant type in wso2apisuite, provide your API provider's **Authorization Endpoint**, along with the **Username** and **Password**. Occasionally, you may also be required to supply a Client ID and Secret.
...
To configure the `Implicit` grant type in wso2apisuite, make sure the [Callback URL](#oauth-callback-url-configuration) is set as required (either `http://172.16.111.83:3333/oauth` or `<your-domain>/oauth`).  You will need to further provide the **Authorization Endpoint** and **Client ID** to generate the token.

### <Icon icon="check" iconType="solid" /> OAuth Callback URL Configuration
When using OAuth 2.0 with wso2apisuite Cloud, the **callback URL** is fixed to `http://172.16.111.83:3333/oauth` while for self-hosted editions of wso2apisuite, the callback URL should be set to `<your-domain>/oauth` . You must configure this URL as a valid **redirect URI** in your OAuth provider's settings. This is important because wso2apisuite handles all OAuth requests on the client side. When your OAuth provider redirects you to this URL, we process the response to obtain the token or authorization code. Any mismatch between the registered callback URL and what wso2apisuite uses will result in errors like `INVALID_REDIRECT_URI` during the authorization process.
