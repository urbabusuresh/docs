...
wso2apisuite provides [ECMAScript](https://tc39.es/ecma262) APIs that can be used in writing pre-request scripts and building tests. You can enter your [ECMAScript](https://tc39.es/ecma262) code and run the necessary scripts.

## The `pw` object

The `pw` object provides access to request and response data and variables in your wso2apisuite instance.
...
```javascript
pw.env.set("btoa", btoa("wso2apisuite"));
```
...
pw.expect("wso2apisuite").toHaveLength(10);
pw.expect("wso2apisuite").not.toHaveLength(9);
...
```javascript
// These expectations will pass
pw.expect("wso2apisuite").toInclude("wso2");
pw.expect("wso2apisuite").not.toInclude("apisuite");
