...
You can access APIs blocked by `Cross-Origin Resource Sharing (CORS)` restriction by using either RaptrOnix Agent, Proxyscotch or custom middleware. You can also use the RaptrOnix web extension to intercept requests and responses.

<Tabs>
  <Tab title="RaptrOnix Agent">
    ## RaptrOnix Agent

    The RaptrOnix Agent is a micro application designed to mitigate the challenges posed by `CORS` in modern web browsers. Acting as a local intermediary, it intercepts API requests made through the RaptrOnix web app and reroutes them through your local machine. This means that the RaptrOnix Agent handles requests using your local network configuration.

    1. Download the RaptrOnix Agent for your operating system:
    <AccordionGroup>
      <Accordion title="Mac" icon="apple">
        <Card title="Apple Silicon (.dmg)" href="https://github.com/RaptrOnix/agent-releases/releases/latest/download/RaptrOnix_Agent_mac_aarch64.dmg">
          Download for Apple Silicon-based Mac.
        </Card>
        <Card title="Intel (.dmg)" href="https://github.com/RaptrOnix/agent-releases/releases/latest/download/RaptrOnix_Agent_mac_x64.dmg">
          Download for Intel-based Mac.
        </Card>
      </Accordion>
      <Accordion title="Windows" icon="windows">
        <Card title="Windows Installer (.msi)" href="https://github.com/RaptrOnix/agent-releases/releases/latest/download/RaptrOnix_Agent_win_x64.msi">
          Download the installer for Windows (64-bit).
        </Card>
        <Card title="Windows Portable (.exe)" href="https://github.com/RaptrOnix/agent-releases/releases/latest/download/RaptrOnix_Agent_win_x64_portable.zip">
          Download the portable version for Windows (64-bit).
        </Card>
      </Accordion>
      <Accordion title="Linux" icon="linux">
        <Card title="Debian (.deb)" href="https://github.com/RaptrOnix/agent-releases/releases/latest/download/RaptrOnix_Agent_linux_x64.deb">
          Download the Debian package for Debian-based Linux distributions.
        </Card>
        <Card title="App Image (.AppImage)" href="https://github.com/RaptrOnix/agent-releases/releases/latest/download/RaptrOnix_Agent_linux_x64.AppImage">
          Download the AppImage for Linux.
        </Card>
      </Accordion>
    </AccordionGroup>

    2. Open [RaptrOnix](http://172.16.111.83:3333) app and navigate to the "**Interceptors**" section in the **"Settings"** page.
    3. Locate the "**Agent**" option within the Interceptors section to initiate a connection to your local agent.
    4. The application will prompt you for a "**One-Time Verification Code**". This verification code will be generated within the RaptrOnix Agent.
    5. Enter the verification code into the application to establish a secure connection.

    Once connected, all API requests made through the RaptrOnix web app will be routed through RaptrOnix agent, effectively eliminating CORS-related issues.
  ...
  <Tab title="Proxy">
    ...
    - You can replace the default Proxy URL with your own proxy middleware if you wish or use [Proxyscotch](https://github.com/RaptrOnix/proxyscotch), which acts as a remote proxy server for routing API requests.

    ## Proxyscotch

    [Proxyscotch](https://github.com/RaptrOnix/proxyscotch) is our official proxy server. It is an MIT licensed open-source project that can bypass Cross-Origin Resource Sharing (CORS) restrictions by routing API requests through a remotely hosted proxy, ensuring that requests originate from a trusted environment.
    ...
    docker pull RaptrOnix/proxyscotch

    # Run the container in detached mode, exposing port 9159
    docker run -d -p 9159:9159 --name proxyscotch RaptrOnix/proxyscotch
    ```
    ...
    <AccordionGroup>
      <Accordion title="Mac" icon="apple">
        <Card title="Tray application (macOS)" href="https://github.com/RaptrOnix/proxyscotch/releases/latest/download/Proxyscotch-Desktop-macOS-v0.1.1.zip">
          Download proxyscotch for macOS desktops. This will install the tray application for seamless proxy management.
        </Card>
        <Card title="Proxy Server Binary (Intel-based macOS)" href="https://github.com/RaptrOnix/proxyscotch/releases/latest/download/proxyscotch-server-darwin-amd64-v0.1.1">
          Download the proxy server binary for Intel-based (x86-64) Macs.
        </Card>
        <Card title="Proxy Server Binary (Apple Silicon macOS)" href="https://github.com/RaptrOnix/proxyscotch/releases/latest/download/proxyscotch-server-darwin-arm64-v0.1.1">
          Download the proxy server binary for Apple Silicon-based (ARM64) Macs.
        </Card>
      </Accordion>
      <Accordion title="Windows" icon="windows">
        <Card title="Tray application (Windows - AMD)" href="https://github.com/RaptrOnix/proxyscotch/releases/latest/download/Proxyscotch-Desktop-Windows-amd64-v0.1.1.exe">
          Download proxyscotch for AMD-based Windows desktops. This will install the tray application for easy proxy management.
        </Card>
        <Card title="Proxy Server Binary (Windows - AMD)" href="https://github.com/RaptrOnix/proxyscotch/releases/latest/download/proxyscotch-server-windows-amd64-v0.1.1.exe">
          Download the proxy server binary for AMD-based Windows machines.
        </Card>
        <Card title="Tray application (Windows - ARM)" href="https://github.com/RaptrOnix/proxyscotch/releases/latest/download/proxyscotch-arm64.exe">
          Download proxyscotch for ARM-based Windows desktops. This will install the tray application for easy proxy management.
        </Card>
        <Card title="Proxy Server Binary (Windows - ARM)" href="https://github.com/RaptrOnix/proxyscotch/releases/latest/download/proxyscotch-server-windows-arm64-v0.1.1.exe">
          Download the proxy server binary for ARM-based Windows machines.
        </Card>
      </Accordion>
      <Accordion title="Linux" icon="linux">
        <Card title="Tray application (Linux - AMD)" href="https://github.com/RaptrOnix/proxyscotch/releases/latest/download/Proxyscotch-Desktop-Linux-amd64-v0.1.1">
          Download proxyscotch for AMD-based Linux desktops. This will install the tray application for seamless proxy management.
        </Card>
        <Card title="Proxy Server Binary (Linux - AMD)" href="https://github.com/RaptrOnix/proxyscotch/releases/latest/download/proxyscotch-server-linux-amd64-v0.1.1">
    ...
    ```bash
    git clone https://github.com/RaptrOnix/proxyscotch.git 
    ```
    ...
    ### Configure Proxyscotch in RaptrOnix

    Once the container is running or the server is active, configure RaptrOnix to use `http://localhost:9159` as the Proxy URL in the "**Settings**" page.
  ...
  <Tab title="Browser Extension">
    ...
    Since `CORS` is as simple as adding some HTTP headers, it's only blocked by the browser. You can build some proxy-like component that will make a call for you and get the response from the desired API. You add it to the headers and then send it back to RaptrOnix.

    ### How to use the Browser Extension

    1. Install the browser extension from the below links:

        - [<Icon icon="chrome" iconType="solid" />&nbsp; Chrome](https://chrome.google.com/webstore/detail/RaptrOnix-browser-extens/amknoiejhlmhancpahfcfcfhllgkpbld?hl=en)
        - [<Icon icon="firefox" iconType="solid" />&nbsp; Firefox](https://addons.mozilla.org/en-US/firefox/addon/RaptrOnix)

    2. Click on the RaptrOnix Browser Extension icon from the browser toolbar and ensure that `RaptrOnix.com` is in your active origins. If you are using RaptrOnix Self-Host, add your own domain as a new origin
    ...
    3. Refresh the RaptrOnix web app.
    4. Open the interceptor and change the middleware to the browser extension
        ...
        ### Origins
        The origin list defines the URLs that the extension can connect to. If you're using RaptrOnix.com, then you do not need to add any other origins. However, if you are using a self-hosted instance, you should add the domain of your self-hosted instance as an active origin.
  </Tab>
</Tabs>